...
(define (render/worm w i)
  (local (; Posn Image -> Image
          ; renders a single segment of the worm to image
          (define (render/body-segment seg img)
            (render/segment seg "red" img)))
    (foldr render/body-segment i w)))
...
(define (get-next-head w)
  (local ((define pos-list (list (list "up" (make-posn 0 -1))
                                 (list "right" (make-posn 1 0))
                                 (list "down" (make-posn 0 1))
                                 (list "left" (make-posn -1 0))))
          (define dir-posn (second (assoc (worm-dir w) pos-list)))
          (define worm-head (first (worm-body w))))
    (make-posn (modulo (+ (posn-x worm-head)
                          (posn-x dir-posn))
                       FIELD-SIZE)
               (modulo (+ (posn-y worm-head)
                          (posn-y dir-posn))
                       FIELD-SIZE))))
